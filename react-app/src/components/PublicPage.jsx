...
const Project = ({ data, idx }) => {
  ...
  const languagePercentages = (languages) => {
    ...
    const languagesInPercentages = Object.entries(
      getRepoLanguagePercentage(languages),
    ).sort((b, a) => a[1] - b[1]);
  ...
  return (
    <>
      ...
      <div className="public-project">
        <Row>
          {" "}
          {/* Name + languages + created */}
          <Col>
            {" "}
            {/* md={{ order: idx % 2 ? "last" : "first" }} */}
            <h1>{data?.name}</h1>
            ...
            <Row className="pt-3">
              ...
              <Col>
                <div>
                  {data?.github
                    ? languagePercentages(data.languages)
                    : plainLanguageList(data.languages)}
                </div>
              </Col>
            </Row>
  ...
  return (
    <Container>
      {projects?.map((project, idx) => (
        <Project key={project.id} data={project} idx={idx} />
      ))}
    </Container>
  );
};
...
const PublicPageContent = () => {
  ...
  const mainInfo = (
    <>
      ...
      <Row>
        <h4>About me:</h4>
        <div style={{ whiteSpace: "pre-line", wordWrap: "normal" }}>
          {about}
        </div>
      </Row>
    </>
...
const PublicPage = () => {
  ...
  return (
    <Suspense fallback={<h2 style={{ color: "white" }}>Loading...</h2>}>
      <Await
        resolve={data.user}
        errorElement={<AppError message="page not found" />}
      >
        <PublicPageContent />
      </Await>
    </Suspense>
